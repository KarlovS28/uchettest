...
export default function LoginPage() {
  ...
  const onLoginSubmit = (values: z.infer<typeof loginSchema>) => {
    ...
    // Имитация входа
    setTimeout(() => {
      alert("Функция входа временно недоступна в демо-режиме");
      setIsLoading(false);
    }, 1000);
  };
  ...
  const onSetupSubmit = (values: z.infer<typeof setupSchema>) => {
    ...
    // Имитация настройки
    setTimeout(() => {
      alert("Функция настройки временно недоступна в демо-режиме");
      setIsLoading(false);
    }, 1000);
  };
    ...
    <div className="flex items-center justify-center min-h-screen p-4">
      <div className="paper max-w-md w-full p-8 rounded-lg">
        ...
        <div className="mt-4 pt-4 border-t text-center text-xs text-muted-foreground">
          <p>Временная версия для демонстрации. Авторизация отключена.</p>
        </div>
      </div>
    </div>
  );
